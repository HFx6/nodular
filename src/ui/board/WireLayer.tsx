...
const Wire = memo(function Wire({ e, d, broken, hot, dimmed, onHot, onHotEnd }: WireProps) {
...
export function WireLayer({ nodes, edges, hot, focus, onHot, onHotEnd, arm }: WireLayerProps) {
  ...
  const routeCached = (a: { x: number; y: number }, b: { x: number; y: number }) => {
    const key = `${a.x | 0},${a.y | 0},${b.x | 0},${b.y | 0}`;
    let d = pathCache.current.get(key);
    if (d === undefined) { d = routeWire(a, b); pathCache.current.set(key, d); }
    return d;
  };
  ...
  return (
    <svg width="1" height="1" style={{ position: "absolute", left: 0, top: 0, overflow: "visible", pointerEvents: "none" }}>
      ...
      {edges.map((e) => {
        ...
        if (!g) return null;
        return (
          <Wire key={e.id} e={e} d={routeCached(g.a, g.b)} broken={g.broken}
            hot={hot === e.id} dimmed={focus !== null && focus !== e.id}
            onHot={onHot} onHotEnd={onHotEnd} />
        );
      })}
    </svg>
  );
