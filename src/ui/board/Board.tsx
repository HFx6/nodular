...
type DragState =
  ...
  | {
      kind: "nodes";
      grabs: Array<{ id: string; dx: number; dy: number }>;
      sx: number;
      sy: number;
      moved: boolean;
    }
...
export function Board({
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      e.preventDefault();
      if (e.ctrlKey) {
        zoomAt(e, 1 - e.deltaY * 0.01);
        return;
      } // pinch
      const mouseWheel =
        e.deltaMode !== 0 || (e.deltaX === 0 && Math.abs(e.deltaY) >= 60);
      if (mouseWheel)
        zoomAt(e, 1 - Math.sign(e.deltaY) * 0.12); // wheel notch = zoom step
  ...
  const onHotEnd = useCallback(
    (id: string) => setHot((h) => (h === id ? null : h)),
    [],
  );
  ...
  const capture = (pid: number) => {
    try {
      boardRef.current?.setPointerCapture(pid);
    } catch {
  ...
  const setDragCursor = (c: "grabbing" | "nwse-resize" | null) => {
    const el = boardRef.current;
    if (!el) return;
    if (c) el.dataset.drag = c;
    else delete el.dataset.drag;
  };
  ...
  const hitTest = (m: Marquee): string[] => {
    ...
    for (const [id, el] of cardRefs.current) {
      ...
      const x = (b.left - r.left - view.x) / view.k,
        y = (b.top - r.top - view.y) / view.k;
      const w = b.width / view.k,
        h = b.height / view.k;
  ...
  const nodeDown = (e: ReactPointerEvent, id: string) => {
    if (e.shiftKey) {
      useGraphStore.getState().select(id, true);
      return;
    }
    const p = toBoard(e);
    ...
    drag.current = {
      ...
      grabs: ids
        .filter((i) => nodes[i])
        .map((i) => ({ id: i, dx: p.x - nodes[i]!.x, dy: p.y - nodes[i]!.y })),
      sx: e.clientX,
      sy: e.clientY,
      moved: false,
    };
  ...
  const commitNodes = (
    d: Extract<DragState, { kind: "nodes" }>,
    p: { x: number; y: number },
  ) => {
    useGraphStore.getState().moveNodes(
      d.grabs.map(({ id, dx, dy }) => ({
        id,
  ...
  const resizeStart = (e: ReactPointerEvent, id: string) => {
    ...
    drag.current = {
      ...
      oy: p.y,
      w: n.w,
      h: n.h ?? el.getBoundingClientRect().height / view.k,
      widthOnly,
    };
    capture(e.pointerId);
  ...
  const move = (e: ReactPointerEvent) => {
    ...
    if (!d) {
      ...
      if (arm && e.buttons === 1) {
        ...
        useGraphStore.getState().setArmDrag(toBoard(e));
      }
      return;
    }
    if (d.kind === "pan") {
    ...
    if (!dragRaf.current) {
      dragRaf.current = requestAnimationFrame(() => {
        ...
        if (dd?.kind === "nodes" && lastPt.current) {
          ...
          if (!histPaused.current) {
            pauseHistory();
            histPaused.current = true;
          }
        }
      });
    }
  ...
  const up = (e: ReactPointerEvent) => {
    ...
    if (drag.current?.kind === "nodes") {
      ...
      if (drag.current.moved && lastPt.current)
        commitNodes(drag.current, lastPt.current);
      if (histPaused.current) {
        resumeHistory();
        histPaused.current = false;
      }
    }
  ...
  const ghostNode =
    placing && ghost ? spawnNode("__ghost", ghost, placing) : null;
    ...
    <div
      ...
      onDrop={(e) => {
        ...
        void importFile(f).then((ok) =>
          notify(ok ? `imported ${f.name}` : `couldn't read ${f.name}`),
        );
      }}
      className="board"
      style={{
        cursor: placing ? "copy" : arm ? "crosshair" : "default",
        ...gridLayers,
      }}
    >
      <div
      ...
      >
        ...
        {Object.values(nodes).map((n) => (
          <NodeCard
            ...
            dimmed={
              focusEdge !== null &&
              n.id !== focusEdge.from[0] &&
              n.id !== focusEdge.to[0]
            }
            onHeaderPointerDown={nodeDown}
            onHeaderDoubleClick={onOpenRail}
        ...
        {ghostNode && (
          <div
            className="ghost"
            style={{ left: ghostNode.x, top: ghostNode.y, width: ghostNode.w }}
          >
            <div className="ghost-head">{ghostNode.name}</div>
