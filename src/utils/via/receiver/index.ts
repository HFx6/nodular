...
const toKey = (key: string) =>
	key === "xlinkHref" || key === "xlink:href" ? "href" : normalizeKeySvg(key);
...
class ViaReceiverClass {
	...
	WrapArg<T extends PrimitiveType | Function | Object>(
		arg: T
	): [number, PrimitiveType | T] {
		if (CanStructuredClone(arg)) return [SerializeType.Primitive, arg as T];
		else return [SerializeType.Object, this.ObjectToId(arg)];
	}
	...
	OnMessage(data) {
		switch (data.type) {
			...
			case "error":
				console.log({ data: data.data });
				this.postMessage({ type: "error", payload: data.data });
				break;
			default:
	...
	ViaCall(
		...
		if (methodName === "removeEventListener") {
			...
			(args[0] === "tabIndex" || base["isSVG"])
		) {
			if (args[0] === "tabIndex") {
				//already set in ViaSet
			} else {
				const ret = base.setAttribute(
					toKey(args[0] as string),
					args[1]
				);
				this.idMap.set(returnObjectId, ret);
			}
		} else {
			const ret = base[methodName](
				...args
					.map((a) => (Array.isArray(a) ? [].slice.call(a) : a))
					.flat()
			);
			this.idMap.set(returnObjectId, ret);
		}
	...
	ViaSet(objectId: number, path: string[], valueData: ArgType) {
		...
		if (propertyName === "tabIndex" && typeof value === "undefined")
			...
			else base.setAttribute(key, String(value));
		} else base[propertyName] = value;
	}
	...
	ViaGet(
		...
		objectId: number,
		path: string[],
		getResults: [number, [number, PrimitiveType]][]
	) {
		const obj = this.IdToObject(objectId);
		...
		if (path === null) {
			getResults.push([
				getId,
				this.WrapArg(obj) as [number, PrimitiveType],
			]);
			return;
		}
		...
		const value = base["isSVG"]
			? base.getAttribute(toKey(propertyName))
			: base[propertyName];
