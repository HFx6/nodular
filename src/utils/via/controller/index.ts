...
export class ViaClass {
	...
	private finalizationRegistry =
		typeof FinalizationRegistry === "undefined"
			? null
			: new FinalizationRegistry(this.FinalizeID.bind(this));
	...
	viaObjectHandler() {
		...
		return {
			get(
				target: { /* _objectId */ [__ObjectSymbol]: number },
				property: any,
				receiver: any
			) {
				...
				return THIS.makeProperty(
					target[__ObjectSymbol] /* ._objectId */,
					[property]
				);
			},
			...
			set(
				...
				else if (typeof property === "symbol") debugger;
		...
		return {
			get(
				target: {
					_nextCache: any;
					_path: any[];
					/* _objectId */ [__ObjectSymbol]: number;
				},
				property: symbol,
				receiver: any
			) {
				...
				if (property === __TargetSymbol) return target;
				if (IgnoreSymbols[property]) return target[property];
			...
			set(
				target: { _path: any[]; /* _objectId */ [__ObjectSymbol]: any },
				property: any,
				value: any,
				receiver: any
			) {
				if (IgnoreSymbols[property]) target[property] = value;
				else {
			...
			construct(
				...
				argumentsList: any[],
				newTarget: any
			): { /* _objectId: any */ [__ObjectSymbol]: number } {
	...
	wrapArg(arg, _i?: number, _a?: any[]) {
		...
		if (typeof arg === "function") {
			...
			if (propertyTarget)
				return [
					ArgEnum.ObjectProperty,
					propertyTarget[__ObjectSymbol] /* ._objectId */,
					propertyTarget._path,
				] as ObjectPropertyArgType;

			// Neither symbol applied; assume an ordinary callback function
			return [
				ArgEnum.Callback,
				this.getCallbackId(arg),
			] as CallbackArgType;
		}
...
if (!self.Via) self.Via = new ViaClass() as any;
...
function CanStructuredClone<T>(o: T): o is T {
	...
	return (
		...
		(Array.isArray(o) && o.every((oo) => isProxy(oo))) ||
