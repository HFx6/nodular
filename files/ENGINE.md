...
- **Registries** (`core/registry.ts`): the core dispatches only through `LanguageAdapter` and `NodeDefinition` — never branches on language. Registered: js (expr and module both real — modules compile to a blob-URL ES module, import()ed, with bare npm specifiers rewritten to esm.sh), py (static heuristic until the Pyodide kernel), table, image, import, text, state, canvas.
- **Built-ins on the node contract** (`builtins/`): table (`rows` in, clicked row out on "→" via emit), image (`url` sink), import (package name/URL → loaded module, or `module.default` when `useDefault`, emitted on "→"), text (editor value IS the value, natto's text pane), state (current value on "→", a stable setter — accepting plain values or updater functions — on the named `set` port, natto's state pane), canvas (`render` sink — ƒ(ctx, frame) called in the surface's own rAF loop).
- **Ports records** (`core/types.ts PORTS` symbol): the generic multi-output value shape — a value branded with `PORTS` has every port (including "→") read off it by name in `portValue`. `emitValue(id, value, port?)` can dirty a single port's consumers (state's `set(v)` re-flows value readers without re-running the caller — no echo loop). Same value-shape category as the Module unwrap; no kind branching.
- **Run/manual are real** (`core/engine.ts runNode`): ▷ forces one evaluation (fresh `nanoid()` on demand); a `manual` node is a propagation barrier — it and anything reachable only through it evaluates solely via ▷ (natto's `autorun: false`). Deviation from natto: ▷ shows in auto mode too (force re-eval of nondeterministic nodes).
- **Editor value mode** (`GraphNode.valueMode`, natto's `expressionType`): `auto | expr | body | text` per node, passed opaquely through `instantiate(code, names, mode)` to the adapter. Set in the per-node ⚙ settings popover. `module` is omitted — auto already routes module syntax.
- **Render output** (`GraphNode.renderMode`, natto's `renderOutput`): `default | table | text | html` — a `ValueFace` below the editor renders the node's raw value (raw values now cross to React via a `values` slice in the results store; the seed of NEXT #6 value faces). Table mode accepts `unknown[]` or natto's `{ data, onRowClick }`. DOM/React/graphviz/custom deferred.
- **Add-node palette** (`graph/spawn.ts`): config-driven TopBar split button listing eval/import/text/state/table/image/canvas (natto's pane-type menu).
...
3. ~~**Named-export edges deliver undefined** (only "→" edges carry values) until js-module instantiation exists.~~ **Resolved (NEXT #1):** js-module runs for real; `gatherInputs` delivers named exports via a generic `portValue(value, port)` (`core/engine.ts`) that reads the export off the upstream value, and unwraps `export default` for "→" when the value is a module namespace (`Symbol.toStringTag === "Module"`) — value-shape only, no language branching.
