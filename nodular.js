...
class Canvas {
  ...
  dragElement(canvas, nodewrapper, bezpaths) {
    ...
    function elementDrag(e) {
      ...
      nodewrapper.style.transform = `translate(${nwbc.left - pos1}px, ${
        nwbc.top - pos2
      }px)`;
    }
...
class Overlay {
  ...
  createTray(nodelist, id) {
    ...
    idecode.value = nodelist[id].funcstr();
    ...
    idecode.addEventListener("keydown", function (e) {
      if (e.key == "Tab") {
        e.preventDefault();
  ...
  createZoom() {
    ...
    var zoomamount = document.createElement("div");
    zoomamount.className = "zoomamount";
    zoomamount.innerHTML = "<span>50%</span>";
    zoombox.appendChild(zoomamount);
    var zoomin = document.createElement("div");
    zoomin.className = "zoomin";

	zoomin.addEventListener('click', function (evt) {
		var box = document.querySelector(".zoomamountdiv");
		var sizeform = document.querySelector(".zoombox").getBoundingClientRect().width;
		box.style.width = (box.offsetWidth + sizeform/6) + 'px';
	});
